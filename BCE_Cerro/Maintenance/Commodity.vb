...
Public Class Commodity
    ...
    Private Sub Commodity_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        viewload()
    End Sub
        ...
        Try
            ...
            ElseIf btn_Submit.Text = "Submit" Then
                If commodityid <> Nothing Then
                    ...
                    Else
                        strQuery = "UPDATE tbl_commoditytype SET name = '" & txt_loading.Text & "'
                                    WHERE id = '" & commodityid & "'"
                        Dim cmd As New SqlCommand(strQuery)
                        Dim dt As DataTable = GetData(cmd)

                        MetroMessageBox.Show(BCEParent, "Commodity Type has been successfully updated", "Success", MessageBoxButtons.OK, MessageBoxIcon.Information)
                    End If
                Else
                    If txt_loading.Text = Nothing Then
                        MetroMessageBox.Show(BCEParent, "Must have Commodity Description", "Required", MessageBoxButtons.OK, MessageBoxIcon.Warning)
                    Else
                        strQuery = "INSERT INTO tbl_commoditytype (name) 
                                             VALUES 
                                                    ('" & txt_loading.Text & "')"
                        Dim cmd As New SqlCommand(strQuery)
                        Dim dt As DataTable = GetData(cmd)

                        MetroMessageBox.Show(BCEParent, "Commodity Type has been successfully register", "Success", MessageBoxButtons.OK, MessageBoxIcon.Information)
                    End If
                End If
                commodityid = Nothing
                txt_loading.Text = Nothing
    ...
    Sub viewload()
        Try
            strQuery = "SELECT id, name as 'Commodity Type' FROM tbl_commoditytype"
            Dim cmd As New SqlCommand(strQuery)
            Dim dt As DataTable = GetData(cmd)
    ...
    Private Sub dgv_loading_CellDoubleClick(sender As Object, e As DataGridViewCellEventArgs) Handles dgv_loading.CellDoubleClick
        Try
            ...
            commodityid = dgv_loading.Item(0, i).Value
    ...
    Private Sub btn_Remove_Click(sender As Object, e As EventArgs) Handles btn_Remove.Click
        Try
            ...
            If mbox = vbYes Then
                strQuery = "DELETE FROM tbl_commoditytype
                            WHERE id = '" & commodityid & "'"
                Dim cmd As New SqlCommand(strQuery)
                Dim dt As DataTable = GetData(cmd)

                MetroMessageBox.Show(BCEParent, "Commodity Type has been successfully remove", "Success", MessageBoxButtons.OK, MessageBoxIcon.Information)
                viewload()
                txt_loading.Text = Nothing
                btn_Remove.Enabled = False
                btn_Submit.Enabled = True
